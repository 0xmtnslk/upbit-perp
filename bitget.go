...
func (b *BitgetAPI) OpenLongPosition(symbol string, marginUSDT float64, leverage int) (*OrderResponse, error) {
	...
	// 1. BALANCE VALIDATION - Fast cache check with 10% buffer
	sufficient, err := b.Cache.HasSufficientBalance(marginUSDT)
	if err != nil {
		return nil, fmt.Errorf("balance check failed: %w", err)
	}
	if !sufficient {
		return nil, fmt.Errorf("insufficient balance: %.2f USDT margin required (%.2f USDT with 10%% buffer), check your account", marginUSDT, marginUSDT*1.10)
	}
...
func (bc *BalanceCache) HasSufficientBalance(requiredUSDT float64) (bool, error) {
	...
	// Add 10% buffer for fees, leverage adjustments, and safety margin
	requiredWithBuffer := requiredUSDT * 1.10
	sufficient := available >= requiredWithBuffer

	fmt.Printf("💰 Balance check: %.2f USDT available, %.2f USDT required (%.2f margin + 10%% buffer), sufficient: %v\n",
		available, requiredWithBuffer, requiredUSDT, sufficient)
